...
function attentionList(obj){
	...
	var str="";
	        ...
	        $.each(result.attention,function (i,obj) {
	            ...
	            str+="<li><span><a onclick='openNewPanel()'>"+en+"</a></span></li>";//<span class='time'>"+ time +"</span>
...
function load_info(array,searchMap) {
	...
	if(array.length>0){
            // 加载列表模式的代码
   		$(".view_way").html("<span class='view_method1 simple_view' onclick='tiaozhuan(0)'>图文模式</span><span class='map-view' onclick='changeToMap()' >地图模式</span>")
...
function load_info_img(array,searchMap) {
    ...
    if(array.length>0){
        // 加载列表模式的代码
   		$(".view_way").html("<span class='view_method1 hard_view' onclick='tiaozhuan(1)'>图文模式</span><span class='map-view' onclick='changeToMap()' >地图模式</span>");
        for(var i=0;i<array.length;i++){
            var source=formatterSource(array[i].WATSYSTEMCODE);
            info_str+='<li>' +
...
function getImgSrc() {
    ...
    for(let i = 0;i<$(".company-img-content>img").length;i++){
        ...
        $(".company-img-content>img").eq(i).attr("src","image/archives/"+Math.floor(Math.random()*33+1)+".png")
    }
...
function changeToMap() {
    debugger;
    // window.parent.find("#main-iframe").attr("src","map.html?state='1'&keyValue="+encodeURI(searchValue));
    $('.nav-list',window.parent.document).find('li').removeClass("active");
    $('.nav-list',window.parent.document).find('li').eq(0).addClass("active");
    window.parent.document.getElementById("main-iframe").src="map.html?state='1'&keyValue="+encodeURI(searchValue);
    // map.html?state='1'&keyValue="+encodeURI(value);
    // window.parent.document.getElementsByClassName("nav-list").
